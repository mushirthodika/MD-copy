...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef double CAP(object a, object b):

    cdef double cap = 0.0
    for ia, ca in enumerate(a.coefs):
        for ib, cb in enumerate(b.coefs):
 
            cap += a.norm[ia]*b.norm[ib]*ca*cb*\
...
def complexint(a,lmn1,A,b,lmn2,B):
    ...
    cap_x = np.loadtxt('cap_x.txt') 

    cap_w = np.loadtxt('cap_w.txt') 
    ...
    for ia, ib in zip(cap_x,cap_w):
        ...
        xi1 += (np.power(ia-A[0],l1)*np.power(ia-B[0],l2)*\
                   np.exp(-alpha*np.power(ia-AB1,2))*W)*ib
    ...
    for ia, ib in zip(cap_x,cap_w):
        ...
        xi2 += (np.power(ia-A[1],m1)*np.power(ia-B[1],m2)*\
                   np.exp(-alpha*np.power(ia-AB2,2))*W)*ib
    ...
    for ia, ib in zip(cap_x,cap_w):
        ...
        xi3 += (np.power(ia-A[2],n1)*np.power(ia-B[2],n2)*\
                   np.exp(-alpha*np.power(ia-AB3,2))*W)*ib

    term0 = xi1*E(m1,m2,0,A[1]-B[1],a,b)*\
                E(n1,n2,0,A[2]-B[2],a,b)*np.power(np.pi/(a+b),1.5)
    term1 = xi2*E(n1,n2,0,A[2]-B[2],a,b)*\
                E(l1,l2,0,A[0]-B[0],a,b)*np.power(np.pi/(a+b),1.5)
    term2 = xi3*E(l1,l2,0,A[0]-B[0],a,b)*\
                E(m1,m2,0,A[1]-B[1],a,b)*np.power(np.pi/(a+b),1.5)
