...
def overlapyz(a,lmn1,A,b,lmn2,B):
    ''' Evaluates overlap integral between two Gaussians
        Returns a float.
        a:    orbital exponent on Gaussian 'a' (e.g. alpha in the text)
        b:    orbital exponent on Gaussian 'b' (e.g. beta in the text)
    ...
    S2 = E(m1,m2,0,A[1]-B[1],a,b) # Y
    S3 = E(n1,n2,0,A[2]-B[2],a,b) # Z
    return S2*S3*np.power(np.pi/(a+b),1.5)

def overlapzx(a,lmn1,A,b,lmn2,B):
    ''' Evaluates overlap integral between two Gaussians
        Returns a float.
        a:    orbital exponent on Gaussian 'a' (e.g. alpha in the text)
        b:    orbital exponent on Gaussian 'b' (e.g. beta in the text)
    ...
    return S1*S3*np.power(np.pi/(a+b),1.5)

def overlapxy(a,lmn1,A,b,lmn2,B):
    ''' Evaluates overlap integral between two Gaussians
        Returns a float.
        a:    orbital exponent on Gaussian 'a' (e.g. alpha in the text)
        b:    orbital exponent on Gaussian 'b' (e.g. beta in the text)
    ...
    return S1*S2*np.power(np.pi/(a+b),1.5)
...
def complexint(a,lmn1,A,b,lmn2,B):
        ''' Evaluates one-electron CAP integral between two Gaussians
            Returns a float.
            a :: orbital exponent on Gaussian 'A' ("alpha_mu" in J. Chem. Phys. 115, 6853 (2001))
            b :: orbital exponent on Gaussian 'B' ("alpha_nu" in J. Chem. Phys. 115, 6853 (2001))

            lmn1 :: int tuple containing orbital angular momentum for Gaussian 'A' e.g. (1,0,0) for p_x 
            lmn2 :: int tuple containing orbital angular momentum for Gaussian 'B' e.g. (1,0,0) for p_x 
            A :: List containing origin of Gaussian 'A' e.g. [1.0,2.0,3.0]
            B :: List containing origin of Gaussian 'B' e.g. [1.0,2.0,3.0]
        '''
    ...
    cap_x = np.loadtxt('cap_x.txt') ''' Quadrature points for integration'''

    cap_w = np.loadtxt('cap_w.txt') ''' Quadratue weights for integration'''
    ...
    for ia, ib in zip(cap_x,cap_w):
        ...
        if abs(ia) <= c1:
            ...
            W = (abs(ia)-c1)*(abs(ia)-c1)

        xi1 += (np.power(ia-A[0],l1)*np.power(ia-B[0],l2)*np.exp(-alpha*np.power(ia-AB1,2))*W)*ib
    ...
    for ia, ib in zip(cap_x,cap_w):
        ...
        if abs(ia) <= c2:
            ...
            W = (abs(ia)-c2)*(abs(ia)-c2)

        xi2 += (np.power(ia-A[1],m1)*np.power(ia-B[1],m2)*np.exp(-alpha*np.power(ia-AB2,2))*W)*ib
    ...
    for ia, ib in zip(cap_x,cap_w):
        ...
        if abs(ia) <= c3:
            ...
            W = (abs(ia)-c3)*(abs(ia)-c3)

        xi3 += (np.power(ia-A[2],n1)*np.power(ia-B[2],n2)*np.exp(-alpha*np.power(ia-AB3,2))*W)*ib

    term0 = xi1*overlapyz(a,(l1,m1,n1),A,b,(l2,m2,n2),B)
    term1 = xi1*overlapzx(a,(l1,m1,n1),A,b,(l2,m2,n2),B)
    term2 = xi1*overlapxy(a,(l1,m1,n1),A,b,(l2,m2,n2),B)
...
def CAP(a,b):
    ...
    for ia, ca in enumerate(a.coefs):
        for ib, cb in enumerate(b.coefs):
            ...
            cap += a.norm[ia]*b.norm[ia]*ca*cb*\
                       complexint(a.exps[ia],a.shell,a.origin,\
                       b.exps[ib],b.shell,b.origin)
